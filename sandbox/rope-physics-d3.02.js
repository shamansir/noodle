// CODEPEN: https://codepen.io/shamansir/pen/LENWmze
...
d3.select(document)
  .on("mousedown", (mouseEvent) => {
    ...
    nodes[0].fx = mouseEvent.clientX; // startX;
    nodes[0].fy = mouseEvent.clientY; // startY;
  ...
  .on("mousemove", (mouseEvent) => {
    if (cable) {
      ...
      end.fx = mouseEvent.clientX; // endX();
      end.fy = mouseEvent.clientY; // endY();
