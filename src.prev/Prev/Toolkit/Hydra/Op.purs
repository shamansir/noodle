module Prev.Toolkit.Hydra.Op
  ...
  , textureOrValue
  , v'
  , class HydrateIn, fromHydra
  , class HydrateOut, toHydra
  )
  where
...
{-
-- Hydrate, as a part of Hydra mechanics
--
-- `fromHydra (toHydra x)` in is not required to equal to `Just x`
class Hydrate a where
    fromHydra :: Hydra -> Maybe a
    toHydra :: a -> Hydra
    --TODO: compile :: a -> String
    --TODO: color :: a -> String
...
class HydrateIn a where
    fromHydra :: Hydra -> Maybe a


class HydrateOut a where
    toHydra :: a -> Hydra
...
instance HydrateOut Operation where
    toHydra = Op

instance HydrateIn Value where
    fromHydra (Val b) = Just b
    fromHydra _ = Nothing

instance HydrateOut Value where
    toHydra = Val

instance HydrateIn Modifier where
    fromHydra (Mod b) = Just b
    fromHydra _ = Nothing

instance HydrateOut Modifier where
    toHydra = Mod

instance HydrateIn Texture where
    fromHydra (Tex b) = Just b
    fromHydra _ = Nothing

instance HydrateOut Texture where
    toHydra = Tex

instance HydrateIn Buffer where
    fromHydra (Buf b) = Just b
    fromHydra _ = Nothing

instance HydrateOut Buffer where
    toHydra = Buf

{- instance HydrateIn Source where
    fromHydra (Tex (Texture source _)) = Just source -- FIXME: we lose modifiers here
    fromHydra _ = Nothing -}

instance HydrateOut Source where
    toHydra src = Tex $ Texture src []

instance HydrateIn Hydra where
    fromHydra = Just

instance HydrateOut Hydra where
    toHydra = identity
