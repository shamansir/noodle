module FOOBAR.Test.Files.CodeGenTest.Input.Hydra.Color.Thresh where

import Prelude

import Data.Newtype (class Newtype)
import Effect (Effect)
import HydraTk.Repr.Wrap (WrapRepr(..))
import Noodle.Fn.Process as Fn
import Noodle.Fn.Process as Noodle
import Noodle.Fn.Shape (I, O)
import Noodle.Fn.Shape as Noodle
import Noodle.Fn.Shape.Temperament (Cold, Hot)
import Noodle.Id as NId
import Noodle.Node as Noodle
import Noodle.Repr.HasFallback (class HasFallback)
import Noodle.Toolkit.Families as Noodle
import Noodle.Toolkit.Family as Family
import Noodle.Toolkit.Family as Noodle
import Type.Data.List (type (:>))
import Type.Data.List.Extra (TNil)
import HydraTk.Types as HYDRA
import HydraTk.Repr.Wrap as HW
import Data.Tuple.Nested ((/\))

{- Generated by Noodle Codegen from Hydra NDF file. Group :: color. Family :: thresh.

[[ : color : thresh :: <what:Texture {EMP T} -> threshold:Value {N 0.5} -> tolerance:Value {N 0.1}> => out:Texture {EMP T} /-| out::HYDRA.Filter <what> $ HYDRA.Thresh { <threshold>, <tolerance> } |-/ ]] (#37) -}

_thresh :: NId.Family "thresh"
_thresh = NId.Family

type Inlets =
  (I "what" Hot HYDRA.Texture :> I "threshold" Hot HYDRA.Value :> I "tolerance" Hot HYDRA.Value :> TNil) ::
    Noodle.Inlets

type Outlets = (O "out" HYDRA.Texture :> TNil) :: Noodle.Outlets
type InletsRow = (what :: HYDRA.Texture, threshold :: HYDRA.Value, tolerance :: HYDRA.Value)
type OutletsRow = (out :: HYDRA.Texture)
type Shape = Noodle.Shape Inlets Outlets
newtype State = State Unit
type Process = Noodle.Process State InletsRow OutletsRow WrapRepr Effect
type Node = Noodle.Node "thresh" State InletsRow OutletsRow WrapRepr Effect
type Family = Noodle.Family "thresh" State InletsRow OutletsRow WrapRepr Effect
type F = Noodle.F "thresh" State InletsRow OutletsRow WrapRepr Effect

defaultI :: Record InletsRow
defaultI = { what: HYDRA.Empty, threshold: HYDRA.Number 0.5, tolerance: HYDRA.Number 0.1 }

defaultO :: Record OutletsRow
defaultO = { out: HYDRA.Empty }

defaultSt :: State
defaultSt = State unit

_in_what = Noodle.Inlet :: _ "what"
_in_threshold = Noodle.Inlet :: _ "threshold"
_in_tolerance = Noodle.Inlet :: _ "tolerance"
_out_out = Noodle.Outlet :: _ "out"

family :: Family
family = Family.make _thresh defaultSt (Noodle.Shape :: Shape) defaultI defaultO threshP

makeNode :: Effect Node
makeNode = Family.spawn family

threshP :: Process
threshP = do
  what <- Fn.receive _in_what
  threshold <- Fn.receive _in_threshold
  tolerance <- Fn.receive _in_tolerance
  Fn.send _out_out $ HYDRA.Filter what $ HYDRA.Thresh { threshold, tolerance }

instance HasFallback State where
  fallback = defaultSt

derive instance Newtype State _
