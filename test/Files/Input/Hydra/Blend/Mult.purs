module Test.Files.CodeGenTest.Input.Hydra.Blend.Mult where
...
[[ : blend : mult :: <what:Texture {EMP T} -> with:Texture {EMP T} -> amount:Value {N 1.0}> => out:Texture {EMP T} /-| out::HT.BlendOf { <what>, <with> } $ HT.Mult <amount> |-/ ]] (#51) -}
...
type Node = Noodle.Node "mult" HW.WrapRepr InletsRow OutletsRow WrapRepr Effect
type Family = Noodle.Family "mult" HW.WrapRepr InletsRow OutletsRow WrapRepr Effect
type F = Noodle.F "mult" HW.WrapRepr InletsRow OutletsRow WrapRepr Effect
...
family :: Family
family = Family.make _mult (HW.Value HT.None) (Noodle.Shape :: Shape) defaultI defaultO multP
...
multP = do
  ...
  Fn.send _out_out $ HT.BlendOf { what, with } $ HT.Mult amount
