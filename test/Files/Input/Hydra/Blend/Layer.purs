module Test.Files.CodeGenTest.Input.Hydra.Blend.Layer where
...
[[ : blend : layer :: <what:Texture {EMP T} -> with:Texture {EMP T} -> amount:Value {N 1.0}> => out:Texture {EMP T} /-| out::HT.BlendOf { <what>, <with> } $ HT.Layer <amount> |-/ ]] (#49) -}
...
type Node = Noodle.Node "layer" HW.WrapRepr InletsRow OutletsRow WrapRepr Effect
type Family = Noodle.Family "layer" HW.WrapRepr InletsRow OutletsRow WrapRepr Effect
type F = Noodle.F "layer" HW.WrapRepr InletsRow OutletsRow WrapRepr Effect
...
family :: Family
family = Family.make _layer (HW.Value HT.None) (Noodle.Shape :: Shape) defaultI defaultO layerP
...
layerP = do
  ...
  Fn.send _out_out $ HT.BlendOf { what, with } $ HT.Layer amount
