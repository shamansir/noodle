...
process = do
    ...
    case ViC.toMaybe vicA /\ ViC.toMaybe vicB of
        (Just (MinimalRepr.Int a) /\ Just (MinimalRepr.Int b)) -> do
            State.modify_
                \srepr -> case srepr of
                    MinimalRepr.PState (pState /\ s) -> MinimalRepr.PState $ pState /\ (s <> "-" <> show (a + b))
                    MinimalRepr.StrSt strSt -> MinimalRepr.StrSt $ strSt <> "-" <> show (a + b)
                    _ -> srepr
            RawFn.send (Id.outletR "out") $ ViC.accept $ MinimalRepr.Int $ a + b
        _ -> pure unit
