...
instance ToFn String TextureOrValue where
    ...
    {- Geometry -}
    toFn "rotate"   = fn "rotate" $ vt2 "texture" "angle" "speed"
    toFn "scale"    = fn "scale" $ vt5 "texture" "amount" "x-mult" "y-mult" "offset-x" "offset-y"
    toFn "pixelate" = fn "pixelate" $ vt2 "texture" "pixel-x" "pixel-y"
    toFn "repeat"   = fn "repeat" $ vt4 "texture" "repeat-x" "repeat-y" "offset-x" "offset-y"
    toFn "repeat-x" = fn "repeat-x" $ vt2 "texture" "reps" "offset"
    toFn "repeat-y" = fn "repeat-y" $ vt2 "texture" "reps" "offset"
    toFn "kaleid"   = fn "kaleid" $ vt1 "texture" "n-sides"
    toFn "scroll-x" = fn "scroll-x" $ vt2 "texture" "amount" "speed"
    toFn "scroll-y" = fn "scroll-y" $ vt2 "texture" "amount" "speed"

    {- Color -}
    toFn "posterize" = fn "posterize" $ vt2 "texture" "bins" "gamma"
    toFn "shift"     = fn "shift" $ vt4 "texture" "r" "g" "b" "a"
    ...
    toFn "colorama"  = fn "colorama" $ vt1 "texture" "amount"

    {- Blend -}
    toFn "add"       = fn "add" $ vtt1 "texture" "what" "amount"
    toFn "layer"     = fn "layer" $ vtt0 "texture" "what"
    ...
    toFn "mod-repeat-x" = fn "mod-repeat-x" $ vtt2 "texture" "what" "reps" "offset"
    toFn "mod-repeat-y" = fn "mod-repeat-y" $ vtt2 "texture" "what" "reps" "offset"
    toFn "mod-kaleid"   = fn "mod-kaleid" $ vtt1 "texture" "what" "n-sides"
    toFn "mod-scroll-x" = fn "mod-scroll-x" $ vtt2 "texture" "what" "amount" "speed"
    toFn "mod-scroll-y" = fn "mod-scroll-y" $ vtt2 "texture" "what" "amount" "speed"
    toFn "modulate"     = fn "modulate" $ vtt1 "texture" "what" "amount"
    toFn "mod-scale"    = fn "mod-scale" $ vtt2 "texture" "what" "multiple" "offset"
    toFn "mod-pixelate" = fn "mod-pixelate" $ vtt2 "texture" "what" "multiple" "offset"
    toFn "mod-rotate"   = fn "mod-rotate" $ vtt2 "texture" "what" "multiple" "offset"
    toFn "mod-hue"      = fn "mod-hue" $ vtt1 "texture" "what" "amount"
