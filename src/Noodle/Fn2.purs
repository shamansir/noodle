...
run :: forall i ii o oo state d m. MonadRec m => MonadEffect m => Protocol state is os m -> Fn state is os m -> m (Fn state is os m)
run protocol fn = pure fn -- FIXME
...
current :: forall state is os m. Fn state is os m -> ( state /\ Record is /\ Record os )
current fn = state fn /\ inputs fn /\ outputs fn


set :: forall state is os m. ( state /\ Record is /\ Record os ) -> Fn state is os m -> Fn state is os m
set (Fn name _ _ _ process) ( state /\ inputs /\ outputs ) = Fn name state inputs outputs process
...
inputsShape :: forall state (is :: Row Type) os m g. RL.RowToList is g => Record.Keys g => Fn state is os m -> List InputId
inputsShape (Fn _ _ inputs _ _) = Record.keys (Proxy :: Proxy is) -- Record.keys inputs


outputsShape :: forall state is (os :: Row Type) m g. RL.RowToList os g => Record.Keys g => Fn state is os m -> List OutputId
outputsShape (Fn _ _ _ outputs _) = Record.keys (Proxy :: Proxy os) -- Record.keys outputs
...
shape
    ...
    => Record.Keys g
    => Fn state is os m
    -> List InputId /\ List OutputId
shape fn = inputsShape fn /\ outputsShape fn


dimensions :: forall state is os m. Fn state is os m -> Int /\ Int
dimensions = shape >>> bimap Array.length Array.length


dimensionsBy :: forall state is os m. (InputId -> Boolean) -> (OutputId -> Boolean) -> Fn state is os m -> Int /\ Int
dimensionsBy iPred oPred = shape >>> bimap (Array.filter (Tuple.fst >>> iPred) >>> Array.length) (Array.filter (Tuple.fst >>> oPred) >>> Array.length)
