module Noodle.API.Action.Sequence where
...
import Noodle.API.Action.Apply (Step, apply)
import Noodle.Path as Path
import Noodle.Toolkit (Toolkit, NodeDef)
import Noodle.Util (Canceler)


type ActionList d c n = List (Action d c n) -- TODO: or newtype?
-- type EveryStep d c n = Either NoodleError (Network d c n) -> Effect Unit
-- type EveryAction d c n = Action d c n -> Effect Unit
-- type FoldResult d c n = Either NoodleError (Network d c n)
...
pass :: forall d c n. Either NoodleError (Network d c n) -> Effect Unit
pass = const $ pure unit


-- FIXME: change `Array Action` to `Action`
type Sequencer d c n = RollbackFSM NoodleError (Action d c n) (Network d c n)
...
--     -> Network d c n
--     -> ActionList d c n
--     -> Effect (Covered NoodleError (Network d c n) /\ Canceler)
