module Noodle.Parser.NoodleFile where
...
type NoodleFile = List NoodleFileCommand


data NoodleFileCommand
    = AddPatch P.ToPatch
...
instance noodleCommandEq :: Eq NoodleFileCommand where
    eq _ _ = true


instance noodleCommandShow :: Show NoodleFileCommand where
...
addPatch :: Parser NoodleFileCommand
addPatch = do
    _ <- string "patch"
    delim
...
addNode :: Parser NoodleFileCommand
addNode = do
    _ <- string "node"
    delim
    patchAlias <- nextToken
...
addInlet :: Parser NoodleFileCommand
addInlet = do
    _ <- string "inlet"
    delim
    patchAlias <- nextToken
...
addOutlet :: Parser NoodleFileCommand
addOutlet = do
    _ <- string "outlet"
    delim
    patchAlias <- nextToken
...
connect :: Parser NoodleFileCommand
connect = do
    _ <- string "connect"
    delim
...
-- parser :: forall d. T.Toolkit d -> Parser NoodleFileCommand
-- parser toolkit
--      =  addNode toolkit
--     <|> addPatch toolkit


cmdParser :: Parser NoodleFileCommand
cmdParser
     =  addPatch
    <|> addNode
    <|> addInlet
...
parse :: forall d c n. String -> T.Toolkit d c n -> Either ParseError NoodleFile
