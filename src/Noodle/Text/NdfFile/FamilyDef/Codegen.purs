...
import Noodle.Text.NdfFile.Types (FamilyDefRec, ChannelDef(..), EncodedType, EncodedValue, StateDef(..), familyOf)
...
type OptionsRec repr =
  ...
  , infoComment :: Maybe (Maybe FC.Source -> Id.GroupR -> Id.FamilyR -> String)
  , prepr :: Proxy repr
...
generate :: forall repr. CodegenRepr repr => Options repr -> FamilyDefRec -> String
...
generateModule :: forall repr. CodegenRepr repr => Options repr -> FamilyDefRec -> Module Void
generateModule (Options opts) ndef
  = let (StateDef state) = ndef.state in
  addUserImports_ opts.imports
  $ unsafePartial
  $ codegenModule (opts.familyModuleName ndef.group $ familyOf ndef) do
