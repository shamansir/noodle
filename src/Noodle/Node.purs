...
make family state shape inletsRec outletsRec =
    make_ familyR state rawShape inletsValues outletsValues
    where
...
atInlet _ node = (inlets node :: m (Record is)) <#> Record.get (Proxy :: _ i)
...
atInletFlipped node _ = (inlets node :: m (Record is)) <#> Record.get (Proxy :: _ i)
...
atOutlet _ node = (outlets node :: m (Record os)) <#> Record.get (Proxy :: _ o)
...
atOutletFlipped node _ = (outlets node :: m (Record os)) <#> Record.get (Proxy :: _ o)
