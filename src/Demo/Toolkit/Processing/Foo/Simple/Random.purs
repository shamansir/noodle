module MyToolkit.Simple.Random where

import Prelude

import Demo.Toolkit.Processing.Repr (ProcessingRepr)
import Effect (Effect)
import Noodle.Fn.Process as Fn
import Noodle.Fn.Process as Noodle
import Noodle.Fn.Shape (I, O)
import Noodle.Fn.Shape as Noodle
import Noodle.Fn.Shape.Temperament (Cold, Hot)
import Noodle.Id as NId
import Noodle.Node as Noodle
import Noodle.Toolkit.Families as Noodle
import Noodle.Toolkit.Family as Family
import Noodle.Toolkit.Family as Noodle
import Type.Data.List (type (:>))
import Type.Data.List.Extra (TNil)
import Demo.Toolkit.Processing.Repr as PR

{- Generated by Noodle Codegen from NDF file. Group :: simple. Family :: random.

[[  simple : random :: <bang:Bang -> min:Number -> max:Number> => random:Number ]] (#11) -}

_random :: NId.Family "random"
_random = NId.Family

type Inlets =
  (I "bang" Hot PR.Bang :> I "min" Hot Number :> I "max" Hot Number :> TNil) :: Noodle.Inlets

type Outlets = (O "random" Number :> TNil) :: Noodle.Outlets
type InletsRow = (bang :: PR.Bang, min :: Number, max :: Number)
type OutletsRow = (random :: Number)
type Shape = Noodle.Shape Inlets Outlets
type Process = Noodle.Process ProcessingRepr InletsRow OutletsRow ProcessingRepr Effect
type Node = Noodle.Node "random" ProcessingRepr InletsRow OutletsRow ProcessingRepr Effect
type Family = Noodle.Family "random" ProcessingRepr InletsRow OutletsRow ProcessingRepr Effect
type F = Noodle.F "random" ProcessingRepr InletsRow OutletsRow ProcessingRepr Effect

defaultI :: Record InletsRow
defaultI = { bang: PR.Bang, min: 0.0, max: 0.0 }

defaultO :: Record OutletsRow
defaultO = { random: 0.0 }

_in_bang = Noodle.Inlet :: _ "bang"
_in_min = Noodle.Inlet :: _ "min"
_in_max = Noodle.Inlet :: _ "max"
_out_random = Noodle.Outlet :: _ "random"

family :: Family
family = Family.make _random PR.VNone (Noodle.Shape :: Shape) defaultI defaultO randomP

makeNode :: Effect Node
makeNode = Family.spawn family

randomP :: Process
randomP = pure unit
