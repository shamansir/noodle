module Toolkit.Hydra2.Family.Render.Cli.Feed.FExpression where
...
render :: forall m. Applicative m => MonadEffect m => NodeBoxKey -> FExpression.Node m -> BlessedOp FExpression.State m -- FIXME: why it doesn't work with `sendOut` ??
                ...
                , Core.on TextArea.Submit
                    \_ _ -> do
                        ...
                        Blessed.lift $ case mbFn of
                            Just fn -> Node.sendOutM node FExpression._out_out fn
                            Nothing -> pure unit
                        pure unit
