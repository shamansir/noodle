module Test.Files.CodeGenTest.Input.Hydra.Blend.Sub where
...
[[ : blend : sub :: <what:Texture {EMP T} -> with:Texture {EMP T} -> amount:Value {N 1.0}> => out:Texture {EMP T} /-| out::HT.BlendOf { <what>, <with> } $ HT.Sub <amount> |-/ ]] (#48) -}
...
type Node = Noodle.Node "sub" HW.WrapRepr InletsRow OutletsRow WrapRepr Effect
type Family = Noodle.Family "sub" HW.WrapRepr InletsRow OutletsRow WrapRepr Effect
type F = Noodle.F "sub" HW.WrapRepr InletsRow OutletsRow WrapRepr Effect
...
family :: Family
family = Family.make _sub (HW.Value HT.None) (Noodle.Shape :: Shape) defaultI defaultO subP
...
subP = do
  ...
  Fn.send _out_out $ HT.BlendOf { what, with } $ HT.Sub amount
